...
func generateMLDSASPKI(params mldsa.Parameters, oid asn1.ObjectIdentifier) ([]byte, error) {
...
func buildTestTbsCert(certLabel, label string, intermediateCACert *x509.Certificate, isMldsa bool) ([]byte, *ecdsa.PrivateKey, error) {
	...
	if isMldsa {
		...
		case "PQ_UDS_44":
			...
			spki, err = generateMLDSASPKI(mldsa.MLDSA44(), asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 17})
		case "PQ_UDS_87":
			// ML-DSA-87 OID: 2.16.840.1.101.3.4.3.19
			spki, err = generateMLDSASPKI(mldsa.MLDSA87(), asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 19})
		default:
			return nil, nil, fmt.Errorf("unsupported ML-DSA certificate label %q", certLabel)
		}
	} else {
	...
	if isMldsa {
		var parsedTBS tbsCertificate
		if _, err := asn1.Unmarshal(tbs, &parsedTBS); err != nil {
			return nil, nil, fmt.Errorf("failed to unmarshal TBS for MLDSA patching: %v", err)
		}
		parsedTBS.PublicKey = asn1.RawValue{FullBytes: spki}
		// Both PQ_UDS_44 and PQ_UDS_87 are signed by the ML-DSA-87 DICE ICA key.
		parsedTBS.SignatureAlgorithm = pkix.AlgorithmIdentifier{
			Algorithm: asn1.ObjectIdentifier{2, 16, 840, 1, 101, 3, 4, 3, 19},
		}
		parsedTBS.Raw = nil
		tbs, err = asn1.Marshal(parsedTBS)
		if err != nil {
			return nil, nil, fmt.Errorf("failed to marshal patched MLDSA TBS: %v", err)
		}
	}
...
func BuildTestTBSCerts(mgr *skumgr.Manager, skuName string, certLabels []string) (map[string][]byte, map[string]*ecdsa.PrivateKey, error) {
	...
	for _, kl := range certLabels {
		...
		} else if kl == "PQ_UDS_44" || kl == "PQ_UDS_87" {
		...
		tbs, priv, err := buildTestTbsCert(kl, label, issuerCert, isMldsa)
		if err != nil {
			return nil, nil, err
		}
		tbsCerts[kl] = tbs
