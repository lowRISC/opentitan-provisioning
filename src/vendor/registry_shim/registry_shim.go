...
func StartRegistryBuffer(registryBufferAddress string, enableTLS bool, enableMLKEM bool, enableMLDSA bool, caRootCerts string, serviceCert string, serviceKey string) error {
	opts := grpc.WithInsecure()
	if enableTLS {
		credentials, err := (&grpconn.Config{EnableMLKEMTLS: enableMLKEM, EnableMLDSATLS: enableMLDSA}).LoadClientCredentials(caRootCerts, serviceCert, serviceKey)
		if err != nil {
			return err
		}
